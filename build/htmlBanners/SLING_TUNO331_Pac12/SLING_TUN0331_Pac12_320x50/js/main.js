...
master.set(".logos", { rotationY: -90 })
    ...
    // Frame 2
    .staggerTo(".frame-2-logos", 0.5, { rotationY: 0, ease: Back.easeOut.config(3) }, 0.1)
    .from('#header-1', 1.5, {}) // delay
    .staggerTo(".frame-2-logos", 0.5, { rotationY: -90, ease: Power4.easeIn }, 0.1, 'frame-2-out')
    .to("#header-1", 0.5, { y: bannerHeight, ease: subtleBounceOut }, "frame-2-out")
    // Frame 3
    .from("#header-2", 0.5, { y: -bannerHeight, ease: subtleBounceIn }, "frame-3-in")
    .staggerTo(".logos-frame-3", 0.5, { rotationY: 0, ease: Back.easeOut.config(3) }, 0.1, "frame-3-in")
    .from('#header-1', 1.5, {}) // delay
    .to("#header-2", 0.5, { y: bannerHeight, ease: subtleBounceOut }, "frame-3-out")
    .staggerTo(".logos-frame-3", 0.5, { rotationY: -90, ease: Power4.easeIn }, 0.1, "frame-3-out")
    // Frame 4
    .from("#header-3", 0.5, { y: -bannerHeight, ease: subtleBounceIn }, "frame-4-in")
    .to("#pac12-logo-medium", 0.5, { rotationY: 0, ease: Back.easeOut.config(3) }, "frame-4-in")
    .from('#header-1', 1.5, {}) // delay
    .to("#header-3", 0.5, { y: bannerHeight, ease: subtleBounceOut }, "frame-4-out")

    .from('.price-container-top', 0.5, { opacity: 0, y: 2 }, "frame-5-in")
    .from('#line', 0.5, { scaleY: 0 }, "frame-5-in+=0.1")
    .from('.price-container-bottom', 0.5, { opacity: 0, y: 2 }, "frame-5-in+=0.2")
    .to("#old-price", 0.5, { x: -22, ease: Power3.easeInOut }, "frame-5-in+=0.2")
    .from("#cross-line", 0.35, { scaleX: 0, ease: Power3.easeInOut, transformOrigin: "0 50%" }, "cross")
